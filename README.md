...
Сканирует заданные цели, определяет сервисы и баннеры, сохраняет историю в SQLite и отправляет уведомления в Telegram и по email.

## Функции

- высокоскоростное сканирование через `masscan`
- асинхронный banner grabbing и определение сервиса
- уточнение сервиса и версии через `nmap -sV`
- diff между сканированиями
- хранение данных в БД `SQLite`
- интеграция с Vulners API для первичной оценки CVE-рисков
- уведомления в Telegram
- уведомления по email
- запуск по расписанию через `APScheduler`
- тестовый режим уведомлений `--test-notifications`
...
Тест уведомлений без запуска сканирования:
